...
exports.requestPasswordReset = async (req, res) => {
  try {
    ...
    if (!user.isActive) {
      return res.status(400).json({ message: '账号已停用，请联系管理员' });
    }
...
exports.getPasswordResetRequests = async (req, res) => {
  try {
    ...
    }).select('name email passwordResetRequestedAt department');
...
// @desc    Approve password reset and generate code
...
exports.approvePasswordReset = async (req, res) => {
  try {
    ...
    if (!user.passwordResetRequested) {
      return res.status(400).json({ message: '该用户未请求密码重置' });
    }
...
// @desc    Deny password reset request
// @route   POST /api/auth/deny-password-reset
// @access  Private/Admin
...
exports.validateResetCode = async (req, res) => {
  try {
    ...
    if (!user.passwordResetCode) {
      return res.status(400).json({ message: '无有效的重置验证码' });
    }

    // Check if code is expired
    if (user.passwordResetExpires < Date.now()) {
      return res.status(400).json({ message: '验证码已过期，请重新申请' });
    }
...
// @route   POST /api/auth/perform-reset
// @access  Public
exports.performPasswordReset = async (req, res) => {
  try {
    ...
    if (!email || !code || !newPassword) {
      return res.status(400).json({ message: '请提供所有必填信息' });
    }
    ...
    const user = await User.findOne({ email }).select('+password +passwordResetCode +passwordResetExpires');
