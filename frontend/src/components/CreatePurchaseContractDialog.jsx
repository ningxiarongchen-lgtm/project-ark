...
const CreatePurchaseContractDialog = ({ open, onClose, purchaseOrder, onSuccess }) => {
  ...
  // 当采购订单数据加载时，自动填充供应商信息
  useEffect(() => {
    if (purchaseOrder && open) {
      setFormData(prev => ({
        ...prev,
        contract_name: `${purchaseOrder.order_number} 采购合同`,
        contract_amount: purchaseOrder.total_amount || '',
        counterparty: {
          ...prev.counterparty,
          name: purchaseOrder.supplier?.name || '',
          company: purchaseOrder.supplier?.company || purchaseOrder.supplier?.name || '',
          contact_person: purchaseOrder.supplier?.contact_person || '',
          contact_phone: purchaseOrder.supplier?.contact_phone || '',
          contact_email: purchaseOrder.supplier?.email || '',
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      await contractsAPI.createPurchaseContract(contractData);
      ...
      console.error('创建采购合同失败:', err);
      setError(err.response?.data?.message || '创建采购合同失败');
    } finally {
      setLoading(false);
    }
  };
    ...
    <Dialog open={open} onClose={handleClose} maxWidth="md" fullWidth>
      <DialogTitle>
        ...
        <Typography variant="caption" display="block" color="text.secondary">
          采购订单：{purchaseOrder?.order_number}
        </Typography>
      </DialogTitle>
      ...
      <DialogContent dividers>
        ...
        <Grid container spacing={2}>
          ...
          <Grid item xs={12} sx={{ mt: 2 }}>
            <Typography variant="h6" gutterBottom>供应商信息</Typography>
            <Divider sx={{ mb: 2 }} />
          </Grid>
          ...
          {/* 交付信息 */}
          <Grid item xs={12} sx={{ mt: 2 }}>
            <Typography variant="h6" gutterBottom>交付信息</Typography>
            <Divider sx={{ mb: 2 }} />
          </Grid>
          ...
          <Grid item xs={12}>
            <TextField
              fullWidth
              label="交付地址"
              value={formData.delivery_info.delivery_address}
              onChange={(e) => handleChange('delivery_address', e.target.value, 'delivery_info')}
            />
          </Grid>
          ...
          <Grid item xs={6}>
            <TextField
              ...
              label="期望交付日期"
              type="date"
              value={formData.delivery_info.delivery_date}
              onChange={(e) => handleChange('delivery_date', e.target.value, 'delivery_info')}
              InputLabelProps={{ shrink: true }}
            />
          </Grid>
          ...
          <Grid item xs={6}>
            <FormControl fullWidth>
              <InputLabel>交付方式</InputLabel>
              <Select
                value={formData.delivery_info.delivery_method}
                label="交付方式"
                onChange={(e) => handleChange('delivery_method', e.target.value, 'delivery_info')}
              >
                <MenuItem value="物流">物流</MenuItem>
                <MenuItem value="快递">快递</MenuItem>
                <MenuItem value="自提">自提</MenuItem>
                <MenuItem value="其他">其他</MenuItem>
              </Select>
            </FormControl>
          </Grid>
...
export default CreatePurchaseContractDialog;
